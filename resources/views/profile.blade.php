...
<div class="container mx-auto p-4">
    ...
    <div class="flex items-center gap-4 mb-6">
        ...
        @else
            <div class="w-16 h-16 flex items-center justify-center bg-gray-300 dark:bg-gray-600 rounded-full text-2xl">
                👤
            </div>
        @endif

        <div>
            <h1 class="text-3xl font-extrabold mb-1 text-gray-900 dark:text-white">{{ Auth::user()->name ?? 'Tamu' }}</h1>
            <a href="{{ route('profile.edit') }}" 
               class="px-2 py-1 bg-yellow-500 text-white rounded hover:bg-yellow-600 text-sm">
               ✏️ Edit Profile
            </a>
            @if(Auth::check())
                <p class="text-gray-600 dark:text-gray-400 mt-1">ID : ({{ Auth::user()->id }})</p>
            @endif
        </div>
    </div>
    ...
    @if($posts->count() > 0)
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            @foreach($posts as $post)
                ...
                <div class="post-item border rounded-lg shadow bg-white overflow-hidden transition dark-mode:bg-[#1f1f1f] dark-mode:text-white">
                    {{-- 🔸 Media Preview --}}
                    <div class="w-full h-48 flex items-center justify-center bg-gray-100 dark:bg-gray-700 overflow-hidden transition-colors duration-300">
                        @if($cover)
                        ...
                        @elseif($isMusic)
                            <div class="flex flex-col justify-center items-center text-center w-full h-full text-gray-800 dark:text-white">
                        ...
                        @else
                            <div class="flex flex-col justify-center items-center text-center text-gray-600 dark:text-gray-300 w-full h-full">
                    ...
                    <div class="p-4">
                        ...
                        @if(!empty($post->content))
                            <p class="text-gray-700 dark:text-gray-300 text-sm mb-2 break-words">
